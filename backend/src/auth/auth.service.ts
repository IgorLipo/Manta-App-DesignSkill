...
export class AuthService {
  ...
  async register(data: { email: string; password: string; firstName: string; lastName: string; role?: string }) {
    ...
    const user = await this.prisma.user.create({
      data: { email: data.email, passwordHash, firstName: data.firstName, lastName: data.lastName, role: (data.role as any) || 'OWNER' },
    });
  ...
  async refresh(refreshToken: string) {
    try {
      ...
      const stored = await this.prisma.refreshToken.findFirst({ where: { userId: payload.sub, tokenHash: hash, revokedAt: null } });
  ...
  private async saveRefreshToken(userId: string, token: string) {
    ...
    await this.prisma.refreshToken.create({ data: { userId, tokenHash: hash, expiresAt: new Date(Date.now() + 7 * 86400000) } });
  }
